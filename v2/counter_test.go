...
func TestCountsMethods(t *testing.T) {
	...
	counts.onExclusion()
	assert.Equal(t, Counts{Requests: 5, TotalSuccesses: 2, TotalFailures: 2, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 2}, counts)
...
func TestRollingCountsMethods(t *testing.T) {
	...
	rc.onExclusion(0)
	assert.Equal(t, Counts{Requests: 4, TotalSuccesses: 2, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 1, ConsecutiveFailures: 0}, rc.Counts)
	assert.Equal(t, Counts{Requests: 4, TotalSuccesses: 2, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 1, ConsecutiveFailures: 0}, rc.buckets[rc.index(0)])
	assert.Equal(t, Counts{}, rc.buckets[rc.index(1)])
	...
	rc.onRequest()
	assert.Equal(t, Counts{Requests: 5, TotalSuccesses: 2, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 1, ConsecutiveFailures: 0}, rc.Counts)
	assert.Equal(t, Counts{Requests: 4, TotalSuccesses: 2, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 1, ConsecutiveFailures: 0}, rc.buckets[rc.index(0)])
	assert.Equal(t, Counts{Requests: 1}, rc.buckets[rc.index(1)])

	rc.onSuccess(1)
	assert.Equal(t, Counts{Requests: 5, TotalSuccesses: 3, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 2, ConsecutiveFailures: 0}, rc.Counts)
	assert.Equal(t, Counts{Requests: 4, TotalSuccesses: 2, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 1, ConsecutiveFailures: 0}, rc.buckets[rc.index(0)])
...
func TestRollingCountsGrow(t *testing.T) {
	...
	rc.grow(0) // no change
	assert.Equal(t, uint64(0), rc.age)
	assert.Equal(t, Counts{Requests: 3, TotalSuccesses: 1, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 1}, rc.Counts)
	assert.Equal(t, Counts{Requests: 3, TotalSuccesses: 1, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 1}, rc.bucketAt(0))
	...
	assert.Equal(t, Counts{Requests: 3, TotalSuccesses: 1, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 1}, rc.bucketAt(1))
	...
	assert.Equal(t, uint64(2), rc.age)
	assert.Equal(t, Counts{Requests: 9, TotalSuccesses: 1, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 1}, rc.Counts)
	assert.Equal(t, Counts{Requests: 9}, rc.bucketAt(0))
	assert.Equal(t, Counts{TotalSuccesses: 1, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 1}, rc.bucketAt(1))
	...
	assert.Equal(t, uint64(2), rc.age)
	assert.Equal(t, Counts{Requests: 12, TotalSuccesses: 2, TotalFailures: 2, TotalExclusions: 2, ConsecutiveSuccesses: 0, ConsecutiveFailures: 1}, rc.Counts)
	assert.Equal(t, Counts{Requests: 12, TotalSuccesses: 1, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 1}, rc.bucketAt(0))
	assert.Equal(t, Counts{TotalSuccesses: 1, TotalFailures: 1, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 1}, rc.bucketAt(1))
...
func TestRollingCountsBucketAt(t *testing.T) {
	...
	rc.onExclusion(0)
	assert.Equal(t, Counts{Requests: 5, TotalSuccesses: 1, TotalFailures: 3, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 2}, rc.Counts)
	assert.Equal(t, Counts{Requests: 2, TotalFailures: 1, ConsecutiveFailures: 1}, rc.bucketAt(0))
	assert.Equal(t, Counts{Requests: 3, TotalSuccesses: 1, TotalFailures: 2, TotalExclusions: 1, ConsecutiveSuccesses: 0, ConsecutiveFailures: 1}, rc.bucketAt(1))
