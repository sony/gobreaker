...
func pseudoSleep[T any](cb *CircuitBreaker[T], period time.Duration) {
	if !cb.expiry.IsZero() {
		cb.expiry = cb.expiry.Add(-period)
	}
}

func succeed[T any](cb *CircuitBreaker[T]) error {
...
func succeedLater[T any](cb *CircuitBreaker[T], delay time.Duration) <-chan error {
	ch := make(chan error)
	go func() {
		_, err := cb.Execute(func() (T, error) {
			time.Sleep(delay)
...
func succeed2Step[T any](cb *TwoStepCircuitBreaker[T]) error {
	done, err := cb.Allow()
	if err != nil {
		return err
	}
...
func fail[T any](cb *CircuitBreaker[T]) error {
	msg := "fail"
	_, err := cb.Execute(func() (T, error) {
		var result T
...
func fail2Step[T any](cb *TwoStepCircuitBreaker[T]) error {
	done, err := cb.Allow()
	if err != nil {
		return err
	}
...
func causePanic[T any](cb *CircuitBreaker[T]) error {
	_, err := cb.Execute(func() (T, error) {
		panic("oops")
		var result T
		return result, nil
...
func newCustom[T any]() *CircuitBreaker[T] {
	...
	return NewCircuitBreaker[T](customSt)
}

func newNegativeDurationCB[T any]() *CircuitBreaker[T] {
	var negativeSt Settings
	negativeSt.Name = "ncb"
	negativeSt.Interval = time.Duration(-30) * time.Second
	negativeSt.Timeout = time.Duration(-90) * time.Second

	return NewCircuitBreaker[T](negativeSt)
}

func init() {
	defaultCB = NewCircuitBreaker[any](Settings{})
	customCB = newCustom[any]()
	negativeDurationCB = newNegativeDurationCB[any]()
}
